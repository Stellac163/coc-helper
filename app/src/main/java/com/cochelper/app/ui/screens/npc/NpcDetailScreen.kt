...
@Composable
fun NpcDetailScreen(npcId: Long, navController: NavHostController) {
    val container = LocalContainer.current
    val scope = rememberCoroutineScope()
    val npc by container.npcDao.observeById(npcId).collectAsStateWithLifecycle(null)
    ...
    fun startEditing() {
        name = npcData.name
        content = TextFieldValue(npcData.content)
        editing = true
    }

    fun save() {
        scope.launch { container.npcDao.update(npcData.copy(name = name.trim(), content = content.text)) }
    }
    ...
    Scaffold(
        ...
        topBar = {
            SectionTopBar(
                title = if (editing) "编辑npc" else "npc详情",
            ...
            Column(
                ...
                if (editing) {
                    ...
                    Column(
                        ...
                        Text(
                            npcData.name.ifBlank { "未命名npc" },
                            style = MaterialTheme.typography.headlineSmall,
                            color = MaterialTheme.colorScheme.onSurface,
                        )
                        Spacer(Modifier.height(16.dp))
                        if (npcData.content.isBlank()) {
                            Text(
                                "还没有描述，点击右下角编辑",
    ...
    if (confirmDelete) {
        AlertDialog(
            ...
            title = { Text("删除npc") },
            text = { Text("确定要删除“${npcData.name}”吗？此操作不可撤销。") },
            confirmButton = {
                TextButton(onClick = {
                    scope.launch { container.npcDao.delete(npcData) }
                    confirmDelete = false
