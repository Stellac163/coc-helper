...
fun CombatScreen(navController: NavHostController) {
    ...
    val combatants by container.combatantDao.observeAll().collectAsState(emptyList())
    ...
    if (showAdd) {
        ...
        var ranged by remember { mutableStateOf(false) }
        AlertDialog(
            ...
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        Text("携带远程武器", style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onSurface, modifier = Modifier.weight(1f))
                        Switch(checked = ranged, onCheckedChange = { ranged = it })
                    }
                }
            },
            confirmButton = {
                TextButton(
                    onClick = {
                        if (name.isNotBlank()) {
                            ...
                            scope.launch { container.combatantDao.insert(CombatantEntity(name = name.trim(), dex = d, hp = h, maxHp = h, ranged = ranged)) }
                        }
                        showAdd = false
                    },
...
private fun CombatantEditDialog(
    ...
    var ranged by remember(combatant.id) { mutableStateOf(combatant.ranged) }
    ...
    AlertDialog(
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {
                Column {
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        IconButton(onClick = { hp = (hp - 1).coerceAtLeast(0) }) { Icon(Icons.Filled.Remove, contentDescription = "-1") }
                        Text("${hp}/${combatant.maxHp}", style = MaterialTheme.typography.headlineSmall)
                        IconButton(onClick = { hp = (hp + 1).coerceAtMost(combatant.maxHp) }) { Icon(Icons.Filled.Add, contentDescription = "+1") }
                    }
                }
        ...
        confirmButton = {
            TextButton(onClick = { onSave(combatant.copy(hp = hp, status = status, ranged = ranged)) }) { Text("保存") }
        },
        dismissButton = {
            Row {
