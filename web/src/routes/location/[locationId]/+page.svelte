<script lang="ts">
  ...
  const locationId = Number(page.params.locationId);
  const loc = $derived(repo.locations.getById(locationId));
  ...
  function save() {
    if (!loc) return;
    repo.locations.update({ ...loc, name: name.trim(), content });
  }
...
{#if !loc}
  <EmptyState text="地点不存在或已被删除" />
{:else}
  <div class="page">
    <SectionTopBar
      title={editing ? '编辑地点' : '地点详情'}
      onback={() => history.back()}
    ...
    <div class="page-body detail-body">
      {#if editing}
        <div class="col" style="gap:12px; height:100%;">
          <TextField label="地点名称" bind:value={name} />
          <textarea
      ...
      {:else}
        <div class="detail-view">
          <div class="detail-name">{loc.name || '未命名地点'}</div>
          {#if !loc.content}
            <div class="muted">还没有描述，点击右下角编辑</div>
          {:else}
            <MarkdownText text={loc.content} />
          {/if}
        </div>
      {/if}
    </div>
  ...
  {#if confirmDelete}
    <Dialog title="删除地点" onclose={() => (confirmDelete = false)}>
      {#snippet children()}
        <p>确定要删除“{loc.name}”吗？此操作不可撤销。</p>
      {/snippet}
      {#snippet actions()}
